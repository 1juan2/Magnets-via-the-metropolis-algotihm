...
for case in range(len(Fc2.Values["T"])):
    ...
    M_equil_verf.append(np.abs(np.sum(Matrix_Spin_StateVect)))              # Save magnetization of the initial state

    ##Evolution of the state vector (Metropolis)

    time = np.arange(0, Fc2.Values["total_Time"], 1)

    Final_evol = Fc2.Metropoolis(time, Matrix_Spin_StateVect, E_equil_verf, M_equil_verf, Fc2.Values["Start_quantit_calcu"], Fc2.Values["T"][case])
    ...
    Final_evol_array = np.array(Final_evol[3:])
    ...
    FoldName_AvrValu = "AvrValues"                                      #Create and save the data of the aver energy and magnetization of the state for each temp
    FileName_AvrValu = 'AvrValues' + str(Fc2.Values["T"][case]) + '.txt'
    Path_AvrEner = os.path.join(FoldName_AvrValu, FileName_AvrValu)
    os.makedirs(FoldName_AvrValu, exist_ok=True)
    ...
    Data_check_Equi = np.column_stack((time, Final_evol[1], Final_evol[2]))
    ...
    np.savetxt(Path_AvrEner, Data_check_Equi)      #Save the Average Energy values and magnetization in each state per temperature to check equilibrium to graph